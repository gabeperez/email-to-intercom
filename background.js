...
async function sendAdminMessage({ contactId, subject, htmlBody, bodyPlainText, openConversation = true }) {
  const settings = await getIntercomSettings();
  // Send plain text with newlines so Intercom inbox preserves line breaks (HTML body is often stripped in conversation view)
  const body = bodyPlainText != null ? bodyPlainText : htmlBody;
  const template = bodyPlainText != null ? 'plain' : 'personal';
...
async function handleSendEmail(data) {
  const { toEmail, toName, subject, htmlBody, bodyPlainText, openConversation = true } = data;
  ...
  // 2. Send message (prefer bodyPlainText so Intercom preserves line breaks in inbox)
